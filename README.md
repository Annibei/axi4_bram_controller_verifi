...
* This project scopes down an original goal — verifying a full 5-channel AXI4 Interconnect Matrix — to a single AXI4 slave first, so the agent/env/scoreboard architecture can be proven correct and reused when the interconnect-level DUT comes online. <br/> 

-----------------------------
GOALS
-----------------------------
* Handshake Protocol Integrity — every channel (AW, W, B, AR, R) must hold to AMBA AXI4 rules: once VALID asserts, it and the associated payload must not change until READY responds. <br/> 
* Transaction-ID-Aware Data Integrity — write and read data must be independently verified against a golden reference model, keyed by transaction ID (AWID/ARID), not just transaction order. <br/> 
* A Reliable Mixed-Language Simulation Flow — Xilinx's BRAM Controller ships as a VHDL/SystemC model under the hood, not plain Verilog. Getting UVM, SystemVerilog, VHDL, and a C++/DPI scoreboard to reliably co-simulate in XSim was a first-class goal in itself, not just a setup detail. <br/> 
* A Foundation for Interconnect-Level Verification — structure the agent, sequences, environment, and scoreboard so that scaling from one AXI4 slave to a multi-master, multi-slave interconnect is additive work, not a rewrite. <br/> 

-----------------------------
KEY ACHIEVEMENTS
-----------------------------
* Built a complete UVM 1.2 environment — agent (driver, monitor, sequencer), environment, scoreboard, sequence library, and base test — around a real AXI4 slave IP rather than a behavioral stub. <br/> 
* Implemented a C++/DPI-C reference-model scoreboard that mirrors the addressable memory in a hash map and validates read data using per-ID FIFOs (pending_reads[id]), so read responses are checked correctly by transaction ID rather than assumed to arrive in issue order — the core mechanism needed for out-of-order matching at the interconnect level later. <br/> 
* Embedded SystemVerilog Assertions directly in the AXI4 interface (axi4_if) covering AWVALID/WVALID payload stability and address-channel deadlock detection, so every test automatically inherits protocol checking without extra wiring. <br/> 
...
* Diagnosed and fixed a Windows-specific DLL-loading crash in the SystemC/DPI runtime by correctly scoping Vivado's xsc linker properties (xsim.elaborate.xsc.more_options) — a non-obvious toolchain issue that silently broke simulation launch. <br/> 
...
* Golden-model scoreboarding in C++, not pure SystemVerilog — write data is pushed into a software memory model on the write channel, and read data is checked against it on the read channel using ID-indexed queues, so the design is ready to validate responses that arrive out of order. <br/> 
...
* Target simulator: AMD Vivado Simulator (XSim) 2026.1, mixed-language (SystemVerilog + VHDL) with SystemC-based DPI co-simulation for the scoreboard. <br/> 
...
* Extend the scoreboard's per-ID matching (already in place for the BRAM controller's read channel) into true concurrent out-of-order verification across multiple interleaved masters. <br/> 
* Add Read-After-Write (RAW) hazard checking — a read landing on an address with a write still in flight through the interconnect pipeline. <br/> 
